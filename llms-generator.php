<?php
/**
 * Plugin Name:       Kandeshop LLMS.txt Generator
 ...
 * Text Domain:       kandeshop-llms-txt-generator
 */
...
function llms_txt_generator_add_admin_menu() {
    add_options_page(
        esc_html__( 'LLMS.txt Generator', 'kandeshop-llms-txt-generator' ),
        esc_html__( 'LLMS.txt Generator', 'kandeshop-llms-txt-generator' ),
        'manage_options',
        'llms_txt_generator',
...
function llms_txt_generator_settings_init() {
    ...
    add_settings_section(
        ...
        esc_html__( '1. Choose AI User-Agents', 'kandeshop-llms-txt-generator' ),
    ...
    add_settings_section(
        ...
        esc_html__( '2. Set AI Usage Policies', 'kandeshop-llms-txt-generator' ),
    ...
    add_settings_section(
        ...
        esc_html__( '3. Set Directory Disallow Rules', 'kandeshop-llms-txt-generator' ),
        'llms_txt_generator_section_disallow_rules_callback',
        'llms_txt_page'
    );

    // User agents.
    add_settings_field('slg_field_gptbot', esc_html__( 'GPTBot (OpenAI)', 'kandeshop-llms-txt-generator' ), 'llms_txt_generator_field_checkbox_callback', 'llms_txt_page', 'slg_section_user_agents', [ 'label_for' => 'slg_field_gptbot', 'option_name' => 'gptbot' ]);
    add_settings_field('slg_field_google_extended', esc_html__( 'Google-Extended (Google AI)', 'kandeshop-llms-txt-generator' ), 'llms_txt_generator_field_checkbox_callback', 'llms_txt_page', 'slg_section_user_agents', [ 'label_for' => 'slg_field_google_extended', 'option_name' => 'google_extended' ]);
    add_settings_field('slg_field_ccbot', esc_html__( 'CCBot (Common Crawl)', 'kandeshop-llms-txt-generator' ), 'llms_txt_generator_field_checkbox_callback', 'llms_txt_page', 'slg_section_user_agents', [ 'label_for' => 'slg_field_ccbot', 'option_name' => 'ccbot' ]);

    // Policies.
    add_settings_field('slg_field_training', esc_html__( 'Training', 'kandeshop-llms-txt-generator' ), 'llms_txt_generator_field_policy_select_callback', 'llms_txt_page', 'slg_section_usage_policies', [ 'label_for' => 'slg_field_training', 'option_name' => 'training' ]);
    add_settings_field('slg_field_summarization', esc_html__( 'Summarization', 'kandeshop-llms-txt-generator' ), 'llms_txt_generator_field_policy_select_callback', 'llms_txt_page', 'slg_section_usage_policies', [ 'label_for' => 'slg_field_summarization', 'option_name' => 'summarization' ]);
    add_settings_field('slg_field_indexing', esc_html__( 'Indexing', 'kandeshop-llms-txt-generator' ), 'llms_txt_generator_field_policy_select_callback', 'llms_txt_page', 'slg_section_usage_policies', [ 'label_for' => 'slg_field_indexing', 'option_name' => 'indexing' ]);
    add_settings_field('slg_field_attribution', esc_html__( 'Attribution', 'kandeshop-llms-txt-generator' ), 'llms_txt_generator_field_policy_select_callback', 'llms_txt_page', 'slg_section_usage_policies', [ 'label_for' => 'slg_field_attribution', 'option_name' => 'attribution' ]);

    // Disallow rules.
    add_settings_field('slg_field_disallow_wp_admin', esc_html__( 'Disallow /wp-admin/', 'kandeshop-llms-txt-generator' ), 'llms_txt_generator_field_checkbox_callback', 'llms_txt_page', 'slg_section_disallow_rules', [ 'label_for' => 'slg_field_disallow_wp_admin', 'option_name' => 'disallow_wp_admin' ]);
    add_settings_field('slg_field_disallow_wp_includes', esc_html__( 'Disallow /wp-includes/', 'kandeshop-llms-txt-generator' ), 'llms_txt_generator_field_checkbox_callback', 'llms_txt_page', 'slg_section_disallow_rules', [ 'label_for' => 'slg_field_disallow_wp_includes', 'option_name' => 'disallow_wp_includes' ]);
    add_settings_field('slg_field_custom_disallow', esc_html__( 'Custom Disallow Rules', 'kandeshop-llms-txt-generator' ), 'llms_txt_generator_field_custom_disallow_callback', 'llms_txt_page', 'slg_section_disallow_rules');
}
...
function llms_txt_generator_section_user_agents_callback() {
    echo '<p>' . esc_html__( 'Select the AI crawlers you wish to add rules for. The policies and rules below will apply to all selected agents.', 'kandeshop-llms-txt-generator' ) . '</p>';
}
function llms_txt_generator_section_usage_policies_callback() {
    echo '<p>' . esc_html__( 'Specify how the selected AI agents are permitted to use your site content.', 'kandeshop-llms-txt-generator' ) . '</p>';
}
function llms_txt_generator_section_disallow_rules_callback() {
    echo '<p>' . esc_html__( 'Select standard paths to block, and add any other custom paths you want to disallow.', 'kandeshop-llms-txt-generator' ) . '</p>';
}
...
function llms_txt_generator_field_policy_select_callback( $args ) {
    ...
    echo '<option value="allow"' . selected($current_value, 'allow', false) . '>' . esc_html__('Allow', 'kandeshop-llms-txt-generator') . '</option>';
    echo '<option value="disallow"' . selected($current_value, 'disallow', false) . '>' . esc_html__('Disallow', 'kandeshop-llms-txt-generator') . '</option>';
    // translators: %s: The policy name (Training, Summarization, etc.).
    echo '<p class="description">' . sprintf( esc_html__( 'Controls if AI can use content for %s.', 'kandeshop-llms-txt-generator' ), '<strong>' . esc_html( ucfirst($option_name) ) . '</strong>' ) . '</p>';
}
function llms_txt_generator_field_custom_disallow_callback() {
    ...
    echo '<p class="description">' . esc_html__( 'Enter one path per line, starting with a slash. e.g., /wp-content/plugins/', 'kandeshop-llms-txt-generator' ) . '</p>';
}
...
function llms_txt_generator_display_admin_notices() {
    ...
    if ( in_array( $notice, [ 'success', 'generated_now_success' ], true ) ) {
        ...
        echo esc_html__( 'Success! The llms.txt file has been generated and uploaded to your site root.', 'kandeshop-llms-txt-generator' ) . esc_html( $size_text ) . ' ';
        // translators: %s: a link to the llms.txt file.
        printf(
            esc_html__( 'You can %s.', 'kandeshop-llms-txt-generator' ),
            sprintf( '<a href="%s" target="_blank">%s</a>', esc_url( $file_url ), esc_html__( 'view it here', 'kandeshop-llms-txt-generator' ) )
        );
        echo '</p></div>';
    } elseif ( 'download_ready' === $notice ) {
        echo '<div class="notice notice-info is-dismissible"><p>' . esc_html__( 'A fresh copy of llms.txt has been prepared for download.', 'kandeshop-llms-txt-generator' ) . '</p></div>';
    } else {
        echo '<div class="notice notice-error is-dismissible"><p>' . esc_html__( 'Error: The llms.txt file could not be created. Please check write permissions to the WordPress root directory.', 'kandeshop-llms-txt-generator' ) . '</p></div>';
    }
...
function llms_txt_generator_options_page_html() {
    ...
    <div class="wrap">
        ...
        <p><?php echo esc_html__( 'This page lets you create an', 'kandeshop-llms-txt-generator' ); ?> <code>llms.txt</code> <?php echo esc_html__( 'file (like robots.txt) to guide LLM/AI crawlers.', 'kandeshop-llms-txt-generator' ); ?></p>

        <form action="options.php" method="post" style="margin-top:1em;">
            <?php
            settings_fields( 'llms_txt_page' );
            do_settings_sections( 'llms_txt_page' );
            submit_button( esc_html__( 'Save Settings & Generate File', 'kandeshop-llms-txt-generator' ) );
            ?>
        </form>

        <hr style="margin: 24px 0;">

        <h2><?php echo esc_html__( 'Actions', 'kandeshop-llms-txt-generator' ); ?></h2>
        <p><?php echo esc_html__( 'Use these tools to generate or download the current llms.txt anytime:', 'kandeshop-llms-txt-generator' ); ?></p>
        ...
        <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="display:inline-block; margin-right:10px;">
            ...
            <?php submit_button( esc_html__( 'Generate Now', 'kandeshop-llms-txt-generator' ), 'primary', 'submit', false ); ?>
        </form>
        ...
        <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" style="display:inline-block;">
            ...
            <?php submit_button( $file_exists ? esc_html__( 'Download Current llms.txt', 'kandeshop-llms-txt-generator' ) : esc_html__( 'Download Preview llms.txt', 'kandeshop-llms-txt-generator' ), 'secondary', 'submit', false ); ?>
        </form>
        ...
        <?php if ( $file_exists ) : ?>
            <p style="margin-top:10px;">
                ...
                // translators: %s: a link to the llms.txt file.
                printf(
                    esc_html__( 'Current file: %s', 'kandeshop-llms-txt-generator' ),
                    sprintf( '<a href="%s" target="_blank">%s</a>', esc_url( home_url( '/llms.txt' ) ), esc_html__( '/llms.txt', 'kandeshop-llms-txt-generator' ) )
                );
                ?>
            </p>
        <?php endif; ?>
...
function llms_txt_generator_handle_generate_now() {
    if ( ! current_user_can( 'manage_options' ) ) {
        wp_die( esc_html__( 'Unauthorized.', 'kandeshop-llms-txt-generator' ) );
    }
